...
const produceUntil = (action, end = forever) =>
...
const produceUntilMatch = (action, matcher) =>
  async function* (seed) {
    while (true) {
...
export const rotateBy = (rotations, step, speed) =>
  (c, i) => {
...
export const combine = (...generators) =>
  (c, i) =>
    async function* () {
...
export const decrypt = async function* (
  msg = "uryybjbeyq",
  charDecryptor = defaultCharAnimation,
) {
  yield msg;
  const decrypted = [...msg];
